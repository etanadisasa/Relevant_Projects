...
# Methods
## Acquiring Data
##### The data we are using is a Water Treatment Plant dataset. It contains several data points. It has 526 observations and 39 variables. 
```{r}
MyData <- read.csv(file = "water-treatment.data", header = FALSE)
str(MyData)
```

##### There are several factor variable that need to be converted to numeric. There are also variables with several missing info/observations. Therefore, we will have to take those out. 
...
MyData <- as.data.frame(sapply(MyData, ConvertToNumer)) #Converting Factor Variables into Numeric
...
##### This graph is not necessarily very helpful when it comes to determining the optimal cluster numbers. It appears, that 5 or 15 may appear the elbow points where the graph turns. This method may be easy, however, it will be on the observer to make a determination. In other words, it is not exact, but very helpful.
...
##### The Gap method had generated an optimal K value of `K = 19` which is different than the value our elbow method had generated. 
...
##### In the above case, the optimal K value is `K = 2`. So, we have a few contenders here from these three different methods of determining the optimal K value. 
...
fit2 <- kmeans(MyData, centers = 10) # 3 cluster solution
fit3 <- kmeans(MyData, centers = 15) # 5 cluster solution
fit4 <- kmeans(MyData, centers = 19) # 6 cluster solution
```
...
Cluster2 <- fviz_cluster(fit2, geom = "point", data = MyData) + ggtitle("A 10 Cluster Solution")
Cluster3 <- fviz_cluster(fit3, geom = "point", data = MyData) + ggtitle("A 15 Cluster Solution")
Cluster4 <- fviz_cluster(fit3, geom = "point", data = MyData) + ggtitle("A 19 Cluster Solution")

grid.arrange(Cluster1, Cluster2, Cluster3, Cluster4, nrow = 2)
```

## Difficult Clusters
##### The 10, 15 and 19 centered clusters are difficult to detect their points or they contain the centroid and a few members in the cluster. However, the 2 cluster solution are much better to visualize and find their centroids. If I have to take one optimal k value, I will consider the 3 cluster solution as it is much better distinctively clusters. 
```{r}
fviz_cluster(fit1, geom = "point", data = MyData) + ggtitle("A 2 Cluster Solution")
```
